...
def get_vendas_por_dia_mes_atual():
    try:
        ...
        # Com o formato ISO (YYYY-MM-DD), strftime('%m', data_hora) funciona nativamente
        cur.execute("""
            SELECT STRFTIME('%d/%m/%Y', data_hora) as data, SUM(valor_total) as total
            FROM vendas
        ...
        for data, total in rows:
            vendas_por_dia.append({
                "data": data, # Formato já vem correto da query (DD/MM/YYYY)
                "valor": round(total, 2)
            })
...
def get_vendas_por_mes_180_dias():
    try:
        ...
        # Agrupar por mes e ano, ordenando cronologicamente
        cur.execute("""
            SELECT 
            ...
            GROUP BY mes
            ORDER BY STRFTIME('%Y%m', data_hora)
        """, (data_inicio,))
